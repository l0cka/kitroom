...
final class AppModel: ObservableObject {
    ...
    @Published private(set) var approvedPackageSources: [ApprovedPackageSource] = []
    ...
    func start() async {
        ...
        if let trustStore = dependencies.packageSourceTrust {
            do {
                approvedPackageSources = try await trustStore.load()
            } catch {
                persistenceWarning = SensitiveValueRedactor.redact(
                    error.localizedDescription
                )
    ...
    func canPlanSkillUninstall(
        ...
        if host.connection == .local {
            return dependencies.localSkillOperations != nil
                && (try? localMutationContext(agent: capability.agent))
                    != nil
        }
        return dependencies.remoteSkillOperations != nil
            && remoteMutationContext(agent: capability.agent) != nil
    }
    ...
    func planSkillUninstall(
        ...
        do {
            ...
            if selectedHost?.connection.isRemote == true {
                guard let context = remoteMutationContext(
                    agent: capability.agent
                ), let engine = dependencies.remoteSkillOperations else {
                    throw OperationProductError.hostDiscoveryRequired
                }
                guard RemotePOSIXPath.deletingLastComponent(
                    physicalOrigin
                ) == context.destinationRoot,
                    let skillName = RemotePOSIXPath.lastComponent(
                        physicalOrigin
                    ) else {
                    throw OperationProductError.targetOutsideUserSkillRoot
                }
                let session = try await dependencies
                    .hostConnectionFactory.connect(to: context.host)
                let plan = try await engine.planUninstall(
                    ...
                    skillName: skillName,
                    remoteDestinationPath: physicalOrigin,
        ...
        do {
            ...
            if host.connection.isRemote {
                guard let context = remoteAgentMutationContext(
                    agent: .claude
                ), let engine = dependencies.remotePluginOperations,
                    let configurationPath = RemotePOSIXPath.appending(
                        relativePath: ".claude/settings.json",
                        to: context.homeDirectory
                    ) else {
                    throw OperationProductError.hostDiscoveryRequired
                }
                let session = try await dependencies.hostConnectionFactory
                ...
                guard let hostIdentity = discovery.identity?.value,
                      let engine = dependencies.nativePluginOperations,
                      let configurationPath = agentConfigurationPath(
                          agent: .claude,
        ...
        case .notInstalled:
            guard supports(
               ...
               PackageSourceTrustPolicy.allowsCodeIntroduction(
                   ...
                   state: state,
                   source: source,
                   approvals: approvedPackageSourceIDs
               ) else {
                return nil
            }
            return .install
    ...
    func isPackageSourceApproved(
        _ source: CatalogSource
    ) -> Bool {
        guard let reference = try? PackageSourceTrustPolicy
            .validatedReference(for: source) else {
            return false
        }
        return approvedPackageSourceIDs.contains(
    ...
    func packageHasRequiredIntegrity(
        package: PackageRecord,
        state: CataloguePackageState?,
        source: CatalogSource
    ) -> Bool {
    ...
    func approvePackageSource(
        _ source: CatalogSource
    ) async {
        operationMessage = nil
        do {
            guard let trustStore = dependencies.packageSourceTrust else {
                throw OperationProductError.operationEngineUnavailable(
                    dependencies.operationIssue
                )
            }
            approvedPackageSources = try await trustStore.approve(
                source: source,
                at: dependencies.clock.now
            )
            operationMessage = "The exact marketplace source is now allowed for digest-backed install and update plans."
        } catch {
            operationMessage = SensitiveValueRedactor.redact(
                error.localizedDescription
            )
    ...
    func revokePackageSource(
        ...
        do {
            ...
            approvedPackageSources = try await trustStore.revoke(
                source: source
            )
            operationMessage = "The marketplace source allowance was removed. Installed packages were not changed."
        } catch {
            operationMessage = SensitiveValueRedactor.redact(
                error.localizedDescription
            )
    ...
    func planCataloguePluginAction(
        ...
        do {
            ...
            if host.connection.isRemote {
                ...
                let session = try await dependencies
                    .hostConnectionFactory.connect(to: host)
                plan = try await engine.planPluginAction(
                    host: host,
                    hostIdentity: context.identity,
                    agentVersion: context.agentVersion,
                    agent: package.agent,
                ...
                guard let context = nativePluginPlanningContext(
                    agent: package.agent
    ...
    func canPlanPluginUninstall(
        ...
        if host.connection == .local {
            return dependencies.nativePluginOperations != nil
                && nativePluginPlanningContext(
                    agent: package.agent
                ) != nil
        }
        return dependencies.remotePluginOperations != nil
            && remoteAgentMutationContext(agent: package.agent) != nil
    }
    ...
    var canPlanCodexMCPAdd: Bool {
        guard let host = selectedHost,
              ...
              supports(.mcpInventory, in: snapshot.capabilities) else {
            return false
        }
        if host.connection == .local {
            return dependencies.nativeMCPOperations != nil
                && nativePluginPlanningContext(agent: .codex) != nil
        }
        return dependencies.remoteMCPOperations != nil
            && remoteAgentMutationContext(agent: .codex) != nil
    }
    ...
    func planAddCodexHTTPServer(
        ...
        do {
            guard canPlanCodexMCPAdd,
                  let host = selectedHost,
                  let snapshot = inventory(for: host, agent: .codex) else {
            ...
            if host.connection.isRemote {
                guard let context = remoteAgentMutationContext(
                    agent: .codex
                ), let engine = dependencies.remoteMCPOperations,
                    let configurationPath = agentConfigurationPath(
                        agent: .codex,
                        homeDirectory: context.homeDirectory
                    ) else {
                ...
                let session = try await dependencies
                    .hostConnectionFactory.connect(to: host)
                plan = try await engine.planAddCodexHTTPServer(
                    host: host,
                    hostIdentity: context.identity,
                    agentVersion: context.agentVersion,
                    serverName: name,
                ...
                guard let context = nativePluginPlanningContext(
                    ...
                    context.configurationPaths.first,
    ...
    func canPlanCodexMCPRemove(
        ...
        guard capability.agent == .codex,
              ...
              let host = selectedHost,
              installation.hostID == host.id else {
            return false
        }
        if host.connection == .local {
            return dependencies.nativeMCPOperations != nil
    ...
    func planRemoveCodexMCPServer(
        ...
        do {
            ...
            if host.connection.isRemote {
                ...
                let session = try await dependencies
                    .hostConnectionFactory.connect(to: host)
                plan = try await engine.planRemoveCodexServer(
                    host: host,
                    hostIdentity: context.identity,
                    agentVersion: context.agentVersion,
                    capability: capability,
    ...
    func applyPendingOperation() async {
        ...
        do {
            ...
            let introducesPluginCode: Bool
            switch plan.execution {
            case let .nativePlugin(spec):
                introducesPluginCode = spec.action == .install
                    || spec.action == .update
            case let .remotePlugin(spec):
                introducesPluginCode = spec.action == .install
                    || spec.action == .update
            default:
                introducesPluginCode = false
            }
            if introducesPluginCode {
                guard isPlanSourceApproved(plan) else {
                    throw OperationProductError.packageSourceNotApproved
                }
            }
            ...
            let session = try await dependencies.hostConnectionFactory
                .connect(to: host)
            let verifiedRemoteDiscovery: HostDiscoverySnapshot?
            if host.connection.isRemote {
                verifiedRemoteDiscovery = try await verifyRemotePlanTarget(
                    plan: plan,
                    host: host,
                    using: session
                )
            } else {
                verifiedRemoteDiscovery = nil
            }
            ...
            let requiresCataloguePreflight: Bool
            switch plan.execution {
            case let .nativePlugin(spec):
                requiresCataloguePreflight =
            ...
            let preflight = OperationPreflight(
                ...
                verifiedHostIdentity: verifiedRemoteDiscovery?
                    .identity?
                    .value,
                verifiedAgentVersion: verifiedRemoteDiscovery?
                    .agents
                    .first(where: {
                        $0.agent == plan.agent
                            && $0.availability == .available
            ...
            case .remoteMCP:
                guard let engine = dependencies.remoteMCPOperations else {
                    throw OperationProductError.operationEngineUnavailable(
                        dependencies.operationIssue
                    )
                }
    ...
    func canDeleteBackup(
        ...
        dependencies.backupRetention != nil
            && BackupRetentionService.canDelete(record)
    }
    ...
    func deleteBackup(
        ...
        operationMessage = nil
        do {
            guard let service = dependencies.backupRetention else {
                throw OperationProductError.operationEngineUnavailable(
                    dependencies.operationIssue
                )
            }
            let updated = try await service.deleteBackup(
                for: record,
                at: dependencies.clock.now
            )
            upsertOperationRecord(updated)
            try await dependencies.persistence.saveOperationRecord(updated)
            operationMessage = "The exact retained local backup was deleted."
        } catch {
            operationMessage = SensitiveValueRedactor.redact(
                error.localizedDescription
            )
    ...
    private func planRemoteSkillInstall(
        ...
        if rootResult.succeeded {
            ...
            guard let parent = RemotePOSIXPath.deletingLastComponent(
                context.destinationRoot
            ) else {
        ...
        guard let destination = RemotePOSIXPath.appending(
            component: sourceDirectory.standardizedFileURL.lastPathComponent,
            to: context.destinationRoot
        ) else {
            throw OperationProductError.remoteTargetUnavailable
        }
        let destinationProbe = try await session.execute(
            CommandRequest(
                executable: "/bin/test",
                arguments: ["-e", destination],
                environment: ["LC_ALL": "C"],
                timeout: .seconds(10),
                maximumOutputBytes: 16_384
            )
        ...
        let plan: OperationPlan
        if destinationProbe.succeeded {
            plan = try await engine.planUpdate(
                host: context.host,
                hostIdentity: context.identity,
                agent: agent,
                agentVersion: context.agentVersion,
        ...
        } else if destinationProbe.exitCode == 1 {
            plan = try await engine.planInstall(
                host: context.host,
                hostIdentity: context.identity,
                agent: agent,
                agentVersion: context.agentVersion,
        ...
        let diskTarget = createsDestinationRoot
            ? RemotePOSIXPath.deletingLastComponent(
                context.destinationRoot
            )
            : context.destinationRoot
        guard let diskTarget else {
            throw OperationProductError.remoteTargetUnavailable
        }
    ...
    private func remoteMutationContext(
        ...
        guard let destinationRoot = RemotePOSIXPath.appending(
            relativePath: relativePath,
            to: base.homeDirectory
        ) else {
            return nil
        }
    ...
    private func verifyRemotePlanTarget(
        ...
        let remoteConfigurationTarget: (
            executablePath: String,
            configurationPath: String
        )?
        switch plan.execution {
        case let .remotePlugin(spec):
            remoteConfigurationTarget = (
                spec.executablePath,
        ...
        default:
            remoteConfigurationTarget = nil
        }
        if let remoteConfigurationTarget {
            guard discovery.agents.contains(where: {
                $0.agent == plan.agent
                    && $0.executablePath
                        == remoteConfigurationTarget.executablePath
            }) else {
                throw OperationProductError.agentVersionChanged
            }
            guard let parent = RemotePOSIXPath.deletingLastComponent(
                remoteConfigurationTarget.configurationPath
            ) else {
                throw OperationProductError.remoteTargetUnavailable
            }
        ...
        guard let destinationRoot = RemotePOSIXPath.deletingLastComponent(
            spec.remoteDestinationPath
        ) else {
            throw OperationProductError.remoteTargetUnavailable
        }
        let permissionTarget: String
        if spec.createsDestinationRoot {
            guard let parent = RemotePOSIXPath.deletingLastComponent(
                destinationRoot
            ) else {
                throw OperationProductError.remoteTargetUnavailable
            ...
            permissionTarget = destinationRoot
        }
    ...
    private func agentConfigurationPath(
        agent: AgentKind,
        homeDirectory: String
    ) -> String? {
        let relativePath = agent == .claude
            ? ".claude/settings.json"
            : ".codex/config.toml"
        return RemotePOSIXPath.appending(
            relativePath: relativePath,
            to: homeDirectory
        )
    }
    ...
    private var approvedPackageSourceIDs: Set<ApprovedPackageSource.ID> {
        Set(approvedPackageSources.map(\.id))
    }

    private func isPlanSourceApproved(
        _ plan: OperationPlan
    ) -> Bool {
        guard let reference = plan.sourceReference else {
            return false
        }
        return approvedPackageSourceIDs.contains(
            ApprovedPackageSource.identifier(
        ...
        ) && plan.contentDigest?.isEmpty == false
    }
...
private func operationTargetMatches(
    ...
    case let .remoteSkill(spec):
        let targetIsReported = snapshot.installations.contains {
            ...
            return RemotePOSIXPath.isNormalizedAbsolute(path)
                && path == spec.remoteDestinationPath
                && installation.origin == .standalone
                && installation.scope == .user
        }
        switch (spec.action, phase) {
...
private func catalogueTargetMatches(
    ...
    guard catalogue.status == .complete,
          ...
          catalogue.capturedAt >= plan.basedOnSnapshotAt else {
        return false
    }
    let selector: String
    switch plan.execution {
...
private enum OperationProductError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .packageSourceNotApproved:
            "The package source is not currently allowed or no package digest is bound to the plan."
        }
    }
}
